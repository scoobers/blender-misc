...
def easy_lattice(lattice_properties):
	...
	if current_object.type == "MESH":
		...
		vertex_grp = current_object.vertex_groups.new(name=VERTEX_GROUP_NAME)
...
class EasyLattice(bpy.types.Operator):
	"""Easy Lattice. If selected object is a mesh lattice will be created based on its selected vertices. To apply lattice either run this operator again with lattice selected, or create new one on a object"""
	...
	lat_w: bpy.props.IntProperty(name="Lattice w", default=2)
